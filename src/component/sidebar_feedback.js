...
import {ulonclick, removeLi, pointer} from '../actions/index'
...
class SideBar extends Component {
    ...
    uulonclick= (id) =>{
    //console.log(id)
    let deltaA = this.props.ulstate.OpenOrClose
    //console.log(deltaA)
    deltaA[id] = !this.props.ulstate.OpenOrClose[id]
                    ...
                    {    //console.log(this.props.record),
                        !this.props.record?"this.props.record.map":Object.keys(this.props.record).map((todo, index)=>{
                            return(
                                ...
                                //console.log(this.props.ulstate.OpenOrClose[this.props.record[todo].id-1]),
                                <li className="subli" key={this.props.record[todo].id}>
                                    <a className="subtitle" style= {{cursor:'pointer',}} id={this.props.record[todo].id} onClick ={e => this.props.pointer(this.props.record[todo].coordinate)}>{this.props.record[todo].name}</a>
                                    <button className="RemoveButton" onClick={(e)=>this.props.remove(this.props.record[todo].id)}>R</button>
                                    <button className="OpenButton" onClick={e => this.uulonclick(this.props.record[todo].id-1)}>O</button>
                                    
                                    <ul className="submenu" style={{display: this.props.ulstate.OpenOrClose[this.props.record[todo].id-1] ? 'block' : 'none' }}>
...
const mapDispatchToProps = dispatch => {
    return {
        ulonclick: data => {
            dispatch(ulonclick(data))
        },
        remove: id => {
            dispatch(removeLi(id))
        },
        pointer: data => {
